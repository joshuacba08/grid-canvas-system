...
function resolveInteger(value: number, minimum: number, name: string): number {
...
function resolveSize(size: GridCanvasSize, name: string): GridCanvasSize {
  return {
...
function resolveRandomValue(random: () => number): number {
  const value = random();
...
function resolveCrossAxisOffset(size: number, align: GridCanvasStackAlign): number {
  switch (align) {
    case "center":
      return -size / 2;
    case "end":
...
export function createParticleBurst(
  ...
  return Array.from({ length: resolvedCount }, () => {
    const direction =
      resolvedAngle - resolvedSpread / 2 + resolvedSpread * resolveRandomValue(random);
...
export function stepParticles(
  ...
  const resolvedGravityX = resolveFiniteNumber(options?.gravityX, 0, "gravityX");
  const resolvedGravityY = resolveFiniteNumber(options?.gravityY, 0, "gravityY");
  ...
  return particles.flatMap((particle, index) => {
    const resolvedParticle = {
      ...
      xSpeed: resolveFiniteNumber(particle.xSpeed, 0, `particles[${index}].xSpeed`),
      ySpeed: resolveFiniteNumber(particle.ySpeed, 0, `particles[${index}].ySpeed`),
      life: resolveNonNegativeNumber(particle.life, 0, `particles[${index}].life`),
      maxLife: resolveNonNegativeNumber(
        particle.maxLife,
        0,
        `particles[${index}].maxLife`,
      ),
      size: resolveNonNegativeNumber(particle.size, 0, `particles[${index}].size`),
    };
    ...
    const nextXSpeed =
      (resolvedParticle.xSpeed + resolvedGravityX * resolvedElapsed) * dragFactor;
    const nextYSpeed =
      (resolvedParticle.ySpeed + resolvedGravityY * resolvedElapsed) * dragFactor;
...
export function layoutStack(
  ...
  const resolvedOrigin = resolvePoint(origin, "origin");
  const direction: GridCanvasStackDirection = options?.direction ?? "vertical";
  ...
  return itemSizes.map((itemSize, index) => {
    ...
    if (direction === "horizontal") {
      const point = {
        x: currentX,
        y: resolvedOrigin.y + resolveCrossAxisOffset(resolvedSize.height, align),
      };
    ...
    const point = {
      x: resolvedOrigin.x + resolveCrossAxisOffset(resolvedSize.width, align),
      y: currentY,
    };
